...
function buildDocumentVaultPath(userId: string, fileName: string) {
  const unique = Date.now().toString(36) + "-" + crypto.randomUUID().slice(0, 8);
  return `${DOCUMENT_VAULT_PREFIX}${userId}/${unique}-${sanitizeUploadFileName(fileName)}`;
}
...
/**
 * Uploads a document vault file directly from the browser to private Vercel Blob.
 * Bypasses the 4.5 MB serverless request body limit on /api/documents/upload.
 */
export async function uploadDocumentVaultClient(
  file: File,
  userId: string,
): Promise<ClientUploadedFile> {
  const validationError = validateUploadFile(file);
  ...
  const pathname = buildDocumentVaultPath(userId, file.name);
  const blob = await upload(pathname, file, {
    access: "private",
    handleUploadUrl: BLOB_UPLOAD_HANDLE_URL,
    multipart: true,
    contentType: file.type || undefined,
    clientPayload: JSON.stringify({ purpose: "document-vault" }),
  });
